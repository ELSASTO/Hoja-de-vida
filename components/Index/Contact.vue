<template>
    <div  align="center" justify="center" >
        <v-row align="center" justify="center">
            <v-col cols="11"  class="contenedor"  align="center" justify="center">
                <v-row align="center" justify="center">
                    <v-col  align="center" justify="center" cols="10" md="5" lg="5" sm="10">
                        <v-img style="mask-image: linear-gradient(black 80%, transparent);" width="450"  src="/IMG/Desarrollador_web_Alejandro_Sastouqe.webp"></v-img>
                    </v-col>
                    <v-col   align="start" justify="center" cols="10" md="5" lg="5" sm="10">
                        
                        <h2  >
                            <span class="font1">
                                SOBRE MI
                            </span><br>
                            <span class="MyName" >DESARROLADOR WEB FULLSTACK</span> 
                        </h2>
                        <p align="center" justify="center"  >
                            <span class="font1" style="font-size: 20px;">"Si lo puedes imaginar, lo puedes programar"</span> 
                            <span style="font-style: italic; font-size: 12px;"><br>-Alejandro Taboada-</span>
                        </p>
                        <v-row>
                            <v-col cols="12" md="6">
                                <div >
                                    <div class="font1" style="color: #ff0000; font-size: 14px;  font-weight: 900;"> NOMBRE </div>
                                    <div class="font1">
                                        Jose Alejandro Sastoque Agudelo
                                    </div>
                                </div>
                                <div >
                                    <div class="font1" style="color: #ff0000; font-size: 14px;  font-weight: 900;"> CORREO ELECTRÓNICO </div>
                                    <div class="font1">
                                        sastoquejose1602@gmail.com
                                    </div>
                                </div>
                                <div >
                                    <div class="font1" style="color: #ff0000; font-size: 14px;  font-weight: 900;"> TELÉFONO </div>
                                    <div class="font1">
                                        +57 314-248-4466
                                    </div>
                                </div>
                            </v-col>
                            <v-col cols="12" md="6">
                                <div >
                                    <div class="font1" style="color: #ff0000; font-size: 14px;  font-weight: 900;"> EDAD </div>
                                    <div class="font1">
                                        24 Años
                                    </div>
                                </div>
                                <div >
                                    <div class="font1" style="color: #ff0000; font-size: 14px;  font-weight: 900;"> EDUCACIÓN </div>
                                    <div class="font1">
                                        Tecnologo en Desarrollo De Software
                                    </div>
                                </div>
                                <div >
                                    <div class="font1" style="color: #ff0000; font-size: 14px;  font-weight: 900;"> HOBBIES</div>
                                    <div class="font1">
                                        Entrenar en el gimnasio, salir a conocer nuevos destinos en moto, aprender cada día más de lo que sé
                                    </div>
                                </div>
                            </v-col>
                        </v-row>
                        <v-btn data-aos="fade-up"
     data-aos-anchor-placement="top-bottom" block class="font1 mt-5" style="background-color: #ff0000;" href="/hoja-de-vida-alejandro-sastoque.pdf">Descargar CV</v-btn>
                    </v-col>
                </v-row>
            </v-col>
        </v-row>
    </div>
</template>